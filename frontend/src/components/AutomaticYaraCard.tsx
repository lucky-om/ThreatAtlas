  ...
  if (crowdsourcedYara && crowdsourcedYara.length > 0) {
    crowdsourcedYara.forEach(cy => {
      evaluatedRules.push({
        ...
        source: 'Crowdsourced',
        author: cy.author || 'Community',
        severity: 'high',
        description: cy.description || 'Community YARA detection rule matched by threat analysis pipeline.',
        matched: true,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span className="material-symbols-outlined" style={{ color: matchCount > 0 ? '#ff2a5f' : 'var(--brand-amber)', fontSize: '20px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '12px', fontFamily: 'var(--font-mono)' }}>
          ...
          <tbody>
            {evaluatedRules.map((rule, idx) => {
              const severityColor = rule.severity === 'critical' ? '#ff2a5f' : rule.severity === 'high' ? '#fb923c' : 'var(--brand)';
              return (
                <tr 
                  key={idx} 
                  style={{ 
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    {rule.matched ? (
                      ...
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#64748b' }}>
                        <span className="material-symbols-outlined" style={{ fontSize: '14px', color: 'var(--brand-amber)' }}>check_circle</span>
                        PASSED
                      </span>
                    )}
                  </td>
