...
export const PhishGuardCard: React.FC<PhishGuardCardProps> = ({ result, url }) => {
  const getRiskColor = (level: string) => {
    switch (level) {
      ...
      case 'Low': return 'var(--brand)';
      default: return 'var(--brand-amber)';
    }
  };
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {result.flags.map((flag, idx) => {
            ...
            const borderColor = isCrit ? 'rgba(255,0,60,0.3)' : isWarn ? 'rgba(245,158,11,0.3)' : isSafe ? 'rgba(0,255,163,0.3)' : 'rgba(255, 69, 0,0.3)';
            const iconColor = isCrit ? '#ff003c' : isWarn ? '#f59e0b' : isSafe ? 'var(--brand-amber)' : 'var(--brand)';
