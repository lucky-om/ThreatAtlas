    ...
    <div style={{ background: '#111927', border: '1px solid #1e293b', borderRadius: '10px', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ color: 'var(--brand)', fontSize: '26px' }}>
            terminal
          </span>
          <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 700, color: report.isPacked ? '#ff2a5f' : 'var(--brand-amber)', marginTop: '4px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 600, color: report.isSigned ? 'var(--brand-amber)' : '#94a3b8', marginTop: '4px' }}>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              padding: '8px 16px',
              color: activeTab === tab.id ? 'var(--brand)' : '#94a3b8',
              ...
              <tbody>
                {report.sections.map((sec, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', background: sec.isSuspicious ? 'rgba(255,42,95,0.05)' : 'transparent' }}>
                    ...
                    <td style={{ padding: '10px 14px', color: sec.entropy > 7.0 ? '#ff2a5f' : sec.entropy > 6.0 ? '#fb923c' : 'var(--brand-amber)', fontWeight: 700 }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <span style={{
                        ...
                        color: sec.isSuspicious ? '#ff2a5f' : 'var(--brand-amber)'
                      }}>
            ...
            {filteredImports.map((imp, idx) => (
              <div key={idx} style={{ background: 'rgba(0,0,0,0.25)', border: `1px solid ${imp.isDangerous ? 'rgba(255,42,95,0.3)' : 'rgba(255,255,255,0.05)'}`, borderRadius: '8px', padding: '12px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontSize: '13px', fontWeight: 700, color: imp.isDangerous ? '#ff2a5f' : 'var(--brand)', fontFamily: 'var(--font-mono)' }}>
                    📂 {imp.library}
                  </span>
      ...
      {activeTab === 'signatures' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {report.signals.map(sig => {
            const sigColor = sig.severity === 'critical' ? '#ff2a5f' : sig.severity === 'high' ? '#fb923c' : 'var(--brand-amber)';
            return (
