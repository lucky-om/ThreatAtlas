...
export const ArchiveForensicsCard: React.FC<ArchiveForensicsCardProps> = ({ report }) => {
  ...
  const threatColor = threatLevel === 'critical' ? '#ff2a5f' : threatLevel === 'suspicious' ? '#fb923c' : threatLevel === 'low' ? '#f59e0b' : 'var(--brand-amber)';
    ...
    <div style={{ background: '#111927', border: '1px solid #1e293b', borderRadius: '10px', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 600, color: hasExecutables || hasScripts ? '#ff2a5f' : 'var(--brand-amber)', marginTop: '4px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 600, color: isEncrypted ? '#fb923c' : 'var(--brand-amber)', marginTop: '4px' }}>
                    ...
                    <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', background: f.isExecutable || f.isScript ? 'rgba(255,42,95,0.05)' : 'transparent' }}>
                      <td style={{ padding: '8px 14px', color: f.isExecutable || f.isScript ? '#ff2a5f' : '#f1f5f9', fontWeight: f.isExecutable ? 700 : 500 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span className="material-symbols-outlined" style={{ fontSize: '16px', color: f.isExecutable ? '#ff2a5f' : 'var(--brand)' }}>
                      ...
                      <td style={{ padding: '8px 14px' }}>
                        {f.isExecutable ? (
                          ...
                          <span style={{ color: 'var(--brand-amber)', fontSize: '11px' }}>
                            ✓ Clean
                          </span>
                        )}
                      </td>
      ...
      {activeTab === 'signals' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {signals.map(sig => {
            const sigColor = sig.severity === 'critical' ? '#ff2a5f' : sig.severity === 'high' ? '#fb923c' : sig.severity === 'medium' ? '#f59e0b' : sig.severity === 'low' ? 'var(--brand)' : 'var(--brand-amber)';
            return (
