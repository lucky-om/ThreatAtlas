...
export const ApkForensicsCard: React.FC<ApkForensicsCardProps> = ({ report }) => {
  ...
  const threatColor = report.threatLevel === 'critical' ? '#ff2a5f' : report.threatLevel === 'suspicious' ? '#fb923c' : report.threatLevel === 'low' ? '#f59e0b' : 'var(--brand-amber)';
    ...
    <div style={{ background: '#111927', border: '1px solid #1e293b', borderRadius: '10px', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ color: 'var(--brand-amber)', fontSize: '26px' }}>
            android
          </span>
          <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--brand-amber)', marginTop: '2px', fontFamily: 'var(--font-mono)' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '15px', fontWeight: 700, color: report.permissions.some(p => p.risk === 'critical') ? '#ff2a5f' : 'var(--brand)', marginTop: '4px' }}>
        ...
        <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid #1e293b', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 600, color: report.isSigned ? 'var(--brand-amber)' : '#ff2a5f', marginTop: '4px' }}>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              padding: '8px 16px',
              color: activeTab === tab.id ? 'var(--brand-amber)' : '#94a3b8',
      ...
      {activeTab === 'permissions' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {report.permissions.map((perm, idx) => {
            const pColor = perm.risk === 'critical' ? '#ff2a5f' : perm.risk === 'dangerous' ? '#fb923c' : 'var(--brand-amber)';
            return (
      ...
      {activeTab === 'components' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px' }}>
          <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px', padding: '16px' }}>
            <div style={{ fontSize: '12px', color: 'var(--brand)', fontWeight: 700, fontFamily: 'var(--font-mono)', marginBottom: '10px' }}>
          ...
          <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px', padding: '16px 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {report.c2Endpoints.map((url, i) => (
                <div key={i} style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--brand)' }}>🌐 {url}</div>
              ))}
            </div>
          </div>
        </div>
