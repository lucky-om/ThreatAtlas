    ...
    <div style={{ background: '#111927', border: '1px solid #1e293b', borderRadius: '10px', padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1e293b', paddingBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span className="material-symbols-outlined" style={{ color: 'var(--brand-amber)', fontSize: '24px' }}>
            verified_user
          </span>
          <div>
        ...
        <div style={{ background: 'rgba(0, 255, 163, 0.1)', border: '1px solid rgba(0, 255, 163, 0.3)', color: 'var(--brand-amber)', padding: '3px 10px', borderRadius: '999px', fontSize: '11px', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>
          ✓ VALID CERTIFICATE
        </div>
      </div>
      ...
      {Array.isArray(sans) && sans.length > 0 && (
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '8px', padding: '14px 18px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {sans.map((s, idx) => (
              <span key={idx} style={{ background: 'rgba(255, 69, 0, 0.08)', border: '1px solid rgba(255, 69, 0, 0.2)', color: 'var(--brand)', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
                {s}
              </span>
            ))}
          </div>
