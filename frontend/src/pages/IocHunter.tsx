...
async function lookupVt(ioc: IocEntry): Promise<Partial<IocResult>> {
  try {
    let endpoint = '';
    if (ioc.type === 'hash') endpoint = `https://www.virustotal.com/api/v3/files/${encodeURIComponent(ioc.ioc)}`;
    else if (ioc.type === 'ip') endpoint = `https://www.virustotal.com/api/v3/ip_addresses/${encodeURIComponent(ioc.ioc)}`;
    else if (ioc.type === 'domain') endpoint = `https://www.virustotal.com/api/v3/domains/${encodeURIComponent(ioc.ioc)}`;
...
async function lookupUrlhaus(ioc: IocEntry): Promise<Partial<IocResult>> {
  try {
    const endpoint = ioc.type === 'url' ? 'https://urlhaus-api.abuse.ch/v1/url/' : 'https://urlhaus-api.abuse.ch/v1/host/';
  ...
  return (
    <>
      <SEO 
        ...
        description="Scan hundreds of IPs, Domains, Hashes, and URLs concurrently against multi-engine threat feeds."
        path="/ioc-hunter"
      />
    <div className="module-page">
      <div className="container" style={{ position: 'relative', zIndex: 10, maxWidth: '1200px', width: '100%' }}>
        ...
        {/* Hero */}
        <PageHeader
          badge="BULK IOC INTELLIGENCE"
          badgeIcon="travel_explore"
          preTitleHighlight="IOC"
          title="HUNTER"
          description="Paste IPs, domains, URLs, or file hashes — cross-reference against multi-engine threat feeds simultaneously."
        />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '24px', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div className="glass-card" style={{ padding: '20px' }}>
              ...
              {[
                { name: 'VirusTotal', desc: '70+ Security Engines', color: 'var(--primary)', icon: 'security' },
                    ...
                    {(['all', 'malicious', 'clean', 'unknown'] as const).map(f => (
                      <button
                        ...
                        style={{
                          ...
                          background: filter === f ? 'rgba(255, 107, 53,0.15)' : 'rgba(255,255,255,0.04)',
