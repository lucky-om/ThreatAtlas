  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      if (inputMode === 'hash') {
        ...
        const res = await fetch(`https://www.virustotal.com/api/v3/files/${encodeURIComponent(hashInput.trim())}`, {
          headers: apiKey ? { 'x-apikey': apiKey } : undefined
        });
        if (!res.ok) throw new Error(`Threat lookup failed: ${res.status === 404 ? 'File not found in threat intelligence database.' : `HTTP ${res.status}`}`);
  ...
  return (
    <div className="module-page">
      <div className="container" style={{ position: 'relative', zIndex: 10, maxWidth: '1300px', width: '100%' }}>
        ...
        {/* Hero */}
        <PageHeader
          badge="YARA PATTERN ENGINE"
          badgeIcon="search_insights"
          preTitleHighlight="YARA"
          title="RULE ENGINE"
          description="Write or load a YARA rule and match it against a file hash or raw text. In-browser pattern matching against real threat signatures."
        />
