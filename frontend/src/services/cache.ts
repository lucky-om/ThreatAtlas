...
export function getCachedItem<T = any>(key: string, ttlMs = DEFAULT_TTL_MS): T | null {
  ...
  } catch (_) { /* localStorage unavailable (private mode / quota) */ }
...
export function setCachedItem<T = any>(key: string, data: T): void {
  ...
  try {
    ...
    try {
      pruneOldCache();
      localStorage.setItem(cleanKey, JSON.stringify(entry));
    } catch (_) { /* storage full after prune — silently skip */ }
  }
}
...
export function clearCache(): void {
  ...
  } catch (_) { /* localStorage unavailable in this environment */ }
}
...
function pruneOldCache(): void {
  ...
  } catch (_) { /* prune failed — storage may be unavailable */ }
}
