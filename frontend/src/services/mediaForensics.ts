...
export function getFileCategory(fileResult?: NormalizedFile | null, rawTarget?: string): FileCategory {
  ...
  // 2. Image formats (High priority: check genuine image extensions, MIME, headers)
  const isImageCandidate = Boolean(
    mime.startsWith('image/') ||
    exifMime.startsWith('image/') ||
    ['jpeg', 'jpg', 'png', 'gif', 'webp', 'bmp', 'tiff', 'tif', 'svg', 'ico', 'heic', 'raw', 'cr2', 'nef', 'avif'].includes(exifFileType) ||
    tags.some(t => ['image', 'jpeg', 'jpg', 'png', 'gif', 'webp', 'svg', 'bmp', 'tiff'].includes(t)) ||
    ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.tiff', '.tif', '.svg', '.ico', '.heic', '.raw', '.cr2', '.nef', '.avif'].some(e => fileName.endsWith(e)) ||
    type.includes('jpeg') || type.includes('png') || type.includes('gif') || type.includes('bitmap') || (type.includes('image') && !type.includes('disc image')) ||
    ...
    (Boolean(exif['ImageWidth']) && Boolean(exif['ImageHeight']) && !['exe', 'dll', 'sys'].includes(exifFileType))
  );

  const hasPeMagic = magic.includes('pe32') || magic.includes('dos executable') || mime.includes('x-dosexec') || mime.includes('x-msdownload');
  if (isImageCandidate && !hasPeMagic) {
    return 'image';
  }

  // 3. Linux ELF Executable
  if (
    type === 'elf' ||
    magic.includes('elf') ||
  ...
  // 4. Windows PE Executable (Strictly genuine PE indicators — never loose substrings)
  const isGenuinePe = Boolean(
    hasPeMagic ||
    ['.exe', '.dll', '.sys', '.scr', '.msi', '.cpl', '.ocx', '.drv', '.efi', '.com'].some(e => fileName.endsWith(e)) ||
    ['exe', 'dll', 'sys'].includes(exifFileType) ||
    type === 'pe' || type === 'pe32' || type === 'pe32+' || /\b(pe32|portable executable|win32 exe)\b/i.test(type) ||
    tags.includes('peexe') || tags.includes('pe') ||
    (fileResult?.extended?.peInfo && (Boolean(fileResult.extended.peInfo.imphash) || (Array.isArray(fileResult.extended.peInfo.sections) && fileResult.extended.peInfo.sections.length > 0)))
  );
