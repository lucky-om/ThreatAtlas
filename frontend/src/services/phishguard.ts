...
const TOP_BRANDS = [
  'paypal', 'microsoft', 'google', 'apple', 'facebook', 'amazon', 'netflix', 
  'instagram', 'twitter', 'steam', 'binance', 'coinbase', 'chase', 'wellsfargo', 
  'telegram', 'discord', 'dropbox', 'linkedin', 'yahoo', 'outlook', 'github'
];
...
export function decodeUrlForensics(rawUrl: string): Array<{ label: string; value: string; danger?: boolean }> {
  ...
  // Step 3: Detect Base64 blobs in query params (credential harvesting payloads)
  ...
  while ((match = b64Pattern.exec(trimmed)) !== null) {
    try {
      ...
      if (/^[\x20-\x7E]+$/.test(decoded)) {
        // High likelihood of credential harvesting if it decodes to an email
        if (decoded.includes('@') && decoded.includes('.')) {
           results.push({ label: '⚠ Base64 Email Payload', value: decoded, danger: true });
        } else {
...
export function analyzeWithPhishGuard(rawUrl: string): PhishGuardResult {
  ...
  // 2. Target Brand Impersonation & Typosquatting (Levenshtein + Regex)
  ...
  for (const brand of BRANDS) {
    for (const pattern of brand.patterns) {
      if (pattern.test(domainOnly)) {
        ...
        flags.push({
          type: 'critical',
          message: `Brand Impersonation (${brand.name}) — domain matches known typosquatting patterns.`,
  ...
  // If no regex match, calculate Levenshtein distance for typosquatting (1 char difference)
  if (!impersonatedBrand) {
    const domainNameWithoutTld = baseDomain.split('.')[0];
    for (const brand of TOP_BRANDS) {
      if (domainNameWithoutTld.length > 3) {
        const distance = levenshteinDistance(domainNameWithoutTld, brand);
        if (distance === 1 && domainNameWithoutTld !== brand) {
          riskScore += 4;
          impersonatedBrand = brand.charAt(0).toUpperCase() + brand.slice(1);
          flags.push({
            type: 'critical',
            message: `Advanced Typosquatting Detected! Domain "${domainNameWithoutTld}" is 1 character away from "${brand}".`,
            category: 'Typosquatting'
          });
        }
      }
  ...
  // 2.5 Punycode & Homograph Attack Detection
  if (domainOnly.includes('xn--')) {
    isHomograph = true;
    riskScore += 5;
    flags.push({
      type: 'critical',
      message: `Punycode IDN Detected (${domainOnly}) — highly indicative of a Homograph attack spoofing a legitimate domain.`,
      category: 'Homograph Attack'
    });
  } else if (/[\u0400-\u04FF]/.test(domainOnly) && /[a-zA-Z]/.test(domainOnly)) {
    // Mixed script (e.g. Cyrillic + Latin)
    isHomograph = true;
    riskScore += 5;
    flags.push({
      type: 'critical',
      message: `Mixed Script Domain Detected — domain mixes Latin and Cyrillic/Greek characters (Homograph attack).`,
      category: 'Homograph Attack'
    });
  }
...
export async function analyzePhishGuardLive(url: string): Promise<PhishGuardResult> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (liveData.isOpenPhishBlacklisted) {
        localResult.riskScore = 10;
        localResult.flags.unshift({
          type: 'critical',
          message: 'ACTIVE THREAT: Domain is currently blacklisted on the global OpenPhish Threat Intel Feed.',
          category: 'Threat Intelligence'
        });
      }
    }
  } catch (err) {
    if (import.meta.env.DEV) console.warn('[PhishGuard Live] Failed to fetch live intel from backend.', err);
  }
  
  // Re-calculate level after live data injection
