...
const PRESET_CLUSTERS: PresetCluster[] = [
  {
    ...
    nodes: [
      ...
      { id: 'killswitch', label: 'www.iuqerfsodp9ifjaposdfjhgosurijfaewrwergwea.com', type: 'domain', verdict: 'suspicious', threatScore: 65, details: 'Sinkholed domain serving as the global execution killswitch.', x: -140, y: -90, radius: 18, color: '#b942ff' },
      ...
      { id: 'tor_onion', label: 'gx7ekbenv2riucmf.onion', type: 'url', verdict: 'malicious', threatScore: 95, details: 'Decryption service payment URL.', x: -70, y: -160, radius: 14, color: '#00f2ff' }
    ],
    links: [
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: 'domain1', label: 'update-microsoft-cloud.live', type: 'domain', verdict: 'malicious', threatScore: 90, details: 'Fast-flux domain spoofing legitimate Microsoft telemetry.', x: 140, y: -90, radius: 17, color: '#b942ff' },
...
export const ThreatGraph: React.FC = () => {
  ...
  const searchTargetGraph = useCallback(async (queryStr: string) => {
    ...
    try {
      if (inType === 'domain' || inType === 'ip') {
        ...
        const res = await fetch(`/api/vt/${typeParam}/${encodeURIComponent(q)}/resolutions?limit=8`);
        if (res.ok) {
        ...
        const res = await fetch(`/api/vt/files/${encodeURIComponent(q)}/contacted_ips?limit=8`);
        if (res.ok) {
          const json = await res.json();
          if (json.data && Array.isArray(json.data)) {
            json.data.forEach((r: any, idx: number) => {
      ...
      // Add default MITRE and OSINT node if few relations
      if (newNodes.length <= 2) {
        newNodes.push({
          id: 'mitre_def',
          label: 'T1071 (Standard Application Layer Protocol)',
          type: 'mitre',
          verdict: 'suspicious',
          threatScore: 60,
          details: 'Adversaries may communicate using standard application layer protocols to avoid detection.',
          x: -120,
          y: 110,
          vx: 0,
          vy: 0,
        ...
        newLinks.push({ source: 'root', target: 'mitre_def', label: 'ATT&CK Vector' });
      }
  ...
  }, [initialQuery, loadPreset, searchTargetGraph]);
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', paddingTop: '70px', background: '#0b111e', color: '#c3c8d4' }}>
      <div className="digital-grid" style={{ opacity: 0.2 }} />
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '0 18px',
                background: 'linear-gradient(135deg, #818cf8, #b942ff)',
                border: 'none',
                borderRadius: '8px',
                color: '#ffffff',
                fontWeight: 700,
      ...
      <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 145px)', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: '16px', left: '16px', display: 'flex', flexDirection: 'column', gap: '8px', zIndex: 10 }}>
          ...
          <div style={{ background: '#111927', border: '1px solid #1e293b', borderRadius: '8px', padding: '4px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', color: physicsRunning ? '#00ffa3' : '#f59e0b', padding: '6px', cursor: 'pointer', borderRadius: '4px' }}
            >
        ...
        <div style={{ position: 'absolute', bottom: '16px', left: '16px', background: 'rgba(17, 25, 39, 0.9)', border: '1px solid #1e293b', borderRadius: '8px', padding: '10px 14px', zIndex: 10, display: 'flex', gap: '14px', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#b942ff' }}></span> Domain / Host</div>
              ...
              {selectedNode.targetLink && (
                <button
                  ...
                  style={{
                    width: '100%',
                    padding: '10px',
                    background: 'linear-gradient(135deg, #00f2ff, #b942ff)',
                    border: 'none',
                    borderRadius: '8px',
                    color: '#0b111e',
                    fontWeight: 700,
