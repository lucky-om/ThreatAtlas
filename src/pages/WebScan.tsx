...
export const WebScan: React.FC = () => {
  ...
  const handleDeepVtScan = async () => {
    ...
    try {
      ...
      setError('VirusTotal scan submission failed. Check API key configuration.');
      setLoading(false);
    }
  };
      ...
      <div className="container" style={{ position: 'relative', zIndex: 10, maxWidth: '1100px', width: '100%' }}>
        ...
        <div className="module-hero" style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div className="module-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.35)', color: '#f59e0b', padding: '4px 14px', borderRadius: '999px', fontFamily: 'var(--font-mono)', fontSize: '11px', fontWeight: 700, marginBottom: '14px' }}>
            ...
            Unified Web, URL &amp; IP Reconnaissance
          </div>
          <h1 className="font-display-lg text-on-surface" style={{ fontSize: '42px', marginBottom: '10px' }}>
            Web, URL &amp; IP <span style={{ color: '#f59e0b' }}>Intelligence</span>
          </h1>
          <p className="font-body-md text-on-surface-variant" style={{ maxWidth: '640px', margin: '0 auto', fontSize: '14px' }}>
            Multi-vector intelligence integrating PhishGuard heuristic scoring, WebFox network recon (DNS, WHOIS, TLS, subdomains), and telecom carrier IP geolocation.
          </p>
        </div>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: scanMode === m.id ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${scanMode === m.id ? 'rgba(245,158,11,0.5)' : 'rgba(255,255,255,0.08)'}`,
                  color: scanMode === m.id ? '#f59e0b' : 'var(--on-surface-variant)',
                }}
              >
        ...
        {!loading && (phishResult || webfoxResult || ipResult || isCarrierIp) && (
          <div className="animate-fade-in-up" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {/* Top Target Meta Bar */}
            <div className="glass-card" style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', borderLeft: `4px solid ${isCarrierIp ? 'var(--primary)' : '#f59e0b'}` }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span className="font-label-caps text-on-surface-variant" style={{ fontSize: '11px' }}>SCANNED ENTITY:</span>
                  <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '4px', background: isCarrierIp ? 'rgba(0,242,255,0.15)' : 'rgba(245,158,11,0.15)', color: isCarrierIp ? 'var(--primary)' : '#f59e0b', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
